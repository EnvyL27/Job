...
<div id="content">
	...
	<div align="right">
		<form method=post>
			<input type="date" name="pencarian">
	...
	<table id="tabel-tampil">
		...
		if($_SERVER['REQUEST_METHOD'] == "POST"){
            ...
            $hari_ini = date('Y-m-d');
			if($pencarian != ""){
				$sql = "SELECT p.id, p.id_pasien, p.tanggal, s.nm_pasien FROM tb_pengunjung p, tb_pasien s 
                        WHERE p.id_pasien = s.id_pasien AND p.tanggal LIKE BETWEEN '%$pencarian%' AND '%$hari_ini'";
	...
	if($_POST['pencarian']!=''){
		...
		<div class="pagination">		
				...
				for($i=1; $i<=$jml_hal; $i++){
					if($i != $hal){
						echo "<a href=\"?p=rekapTanggal&hal=$i\">$i</a>";
					}
					else {
